module maple_display(
    ...
    output reg [11:0] rgb,
    input frame_tick,
    output on,c_maple_image_direction_st,
    output [9:0] maple_left, maple_top,
    input [9:0] monster_left,
    output shuriken_fire
);
    wire attack_btn, left_btn, right_btn;
    assign {attack_btn, right_btn, left_btn} = btn;
    parameter right_end = 639, bottom_end = 479;
    parameter width = 47, height = 49;
    ...
    parameter stand_st = 0, jump_st = 1, fall_st = 2, attack_st = 3;
    ...
    always @(*) begin
        case(c_maple_move_st)
            stand_st: n_maple_move_st = (attack_btn && frame_tick == 1) ? attack_st : !ground ? fall_st : (key == 5'h15 && frame_tick == 1) ? jump_st : stand_st;
            jump_st: n_maple_move_st = (jump_size_index == 8) ? fall_st : jump_st;
            fall_st: n_maple_move_st = (ground) ? stand_st : fall_st;
            attack_st: n_maple_move_st = stand_st;
    ...
    parameter maple_stand_st = 0, maple_walk_st = 1, maple_jump_st = 5, maple_attack_st = 6;
    ...
    always @(*) begin
        case (c_maple_image_st)
            maple_stand_st: begin
                n_maple_image_st = (attack_btn && frame_tick == 1) ? maple_attack_st : (c_maple_move_st == jump_st || c_maple_move_st == fall_st) ? maple_jump_st :
    ...
    wire [11:0] rgb_jump, rgb_walk, rgb_walk1, rgb_walk2, rgb_walk3, rgb_walk4, rgb_stand, rgb_attack, rgb_attack1, rgb_attack2, rgb_attack3;
    ...
    attack1_rom attack1_rom_inst(.clk(clk), .x(x_rom), .y(y_rom), .color_data(rgb_attack1));
    attack2_rom attack2_rom_inst(.clk(clk), .x(x_rom), .y(y_rom), .color_data(rgb_attack2));
    attack3_rom attack3_rom_inst(.clk(clk), .x(x_rom), .y(y_rom), .color_data(rgb_attack3));
    ...
    always @(*) begin
        case(c_maple_image_st)
            ...
            maple_attack_st: rgb = rgb_attack;
    ...
    wire attacking_trig;
    gen_counter #(.SIZE(7552500)) gen_clock_inst1(.clk(clk), .rst(rst), .counter_en(attacking_trig));

    reg [1:0] attacking_state;
    always @(posedge clk or posedge rst) begin
        if (rst) begin 
            attacking_state <= 0;
        end else if (attacking_trig && attack_btn) begin
            attacking_state <= attacking_state + 1;
        end
    end

    assign rgb_attack = (attacking_state == 2'b00) ? rgb_attack1 :
                        (attacking_state == 2'b01) ? rgb_attack2 :
                        (attacking_state == 2'b10) ? rgb_attack3 : 0;

    assign shuriken_fire = attacking_trig && attack_btn;
